...
foreach($rooms as $room) {
	if ((strpos($room['name'], 'Twisty') !== false) || $room['id'] == 2381) {
		$approved[] = $room;
	}
}
$rooms = $approved;
...
$i = 0;
foreach($rooms as $room) {
	$nodes[$i] = array('name' => $room['name'].' ('.$room['id'].')');
	$room_map[$room['id']] = $i;
	$i++;
}
...
$links = array();
foreach($rooms as $room) {
	$roomIndex = $room_map[$room['id']];
	foreach($room['exits'] as $exit) {
		if (!isset($room_map[$exit['dest']])) {
			continue;
		}
		$destIndex = $room_map[$exit['dest']];
		$strength = 0.1;
		if (strpos($room['name'], 'Twisty') !== false) {
			$strength = 1;
		}
		$links[] = array(
			'source' => $roomIndex,
...
echo 'var nodes = '.json_encode($nodes);
echo "\n";
