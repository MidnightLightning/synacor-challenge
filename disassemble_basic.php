...
while ($i < count($memory)) {
	...
	switch ($cmd) {
		case 0:
			...
			break;
		case 1:
			echo 'SET '.valueFormat($memory[$i++]).', '.valueFormat($memory[$i++]);
			break;
		case 2:
		...
		case 3:
			...
			break;
		case 4: // EQ
			echo 'SET '.valueFormat($memory[$i++]).' = ('.valueFormat($memory[$i++]).' == '.valueFormat($memory[$i++]).')? 1 : 0';
			break;
		case 5: // GT
			echo 'SET '.valueFormat($memory[$i++]).' = ('.valueFormat($memory[$i++]).' > '.valueFormat($memory[$i++]).')? 1 : 0';
			break;
		case 6:
			...
			break;
		case 7: // JT
			echo 'IF '.valueFormat($memory[$i++]).' != 0, JMP '.valueFormat($memory[$i++]);
			break;
		case 8: // JF
			echo 'IF '.valueFormat($memory[$i++]).' == 0, JMP '.valueFormat($memory[$i++]);
			break;
